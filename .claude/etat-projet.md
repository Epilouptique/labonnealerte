# LaBonneAlerte — État du projet (24 juillet 2026, fil "vagues d'alertes & PanneauPocket")
...
DB vs CODE (vérifié 24/07, fil PanneauPocket) : écart du 23/07 RÉSOLU — migrate.js passé, sources display_order 433-439 (Deezer→ondes gravitationnelles) en base et enabled. Orphelin veille-artiste-spotify : ABSENT d'init.sql ; init.sql ET migrate.js ne contiennent AUCUN DELETE (migrate = DROP promo_status + exécution d'init.sql, rien d'autre) → la purge ne peut être qu'un DELETE MANUEL dans le shell Railway (réputé fait ce fil ; sinon la ligne subsiste enabled en base, sans trace côté code). Compteurs init.sql (comptés par parse) : 270 lignes sources, 245 enabled (25 désactivées : 7 UPDATE ciblés + fusions en masse vigilance-meteo-%/vacances-zone-*), dont les 3 cartes PanneauPocket (440-442) ; +1 orphelin en base si non purgé → ~245-246 enabled réels. Paramétrées : +2 (panneaupocket, ma-collectivite ; arrosage-canal-gap = broadcast).
...
## Prochain fil
FILE DES CHANTIERS (ordre pressenti) : (1) VAGUES DE SOURCES — XXL 1, XXL 2, 3XL, v2-only, francophonie/plus-value ; (2) cyclones OM (souscription portail « Bulletin Vigilance » à clarifier — cf. liste de courses) ; (3) PERSISTANCE des caches anti-rétroactifs en base (dette qui grossit, aggravée par les 3 caches PanneauPocket) ; (4) streaming phase 2 (SPA anti-bot → headless) ; (5) étape B (lancement). En parallèle/plus tard : (a) fonctionnement + apparence des CARTES et surtout des DECKS (comportement + visuel — relire d'abord identite-visuelle-reference.md) ; (b) DÉBRIEFING STRATÉGIQUE (étapes à venir, opportunités, changements mineurs à trancher). Nettoyages à ne pas oublier : purge de la ligne DB orpheline veille-artiste-spotify (DELETE manuel, migrate.js ne supprime pas), retrait du middleware diag IP dormant (LOG_CLIENT_IP), backlog visuel (dont ligne .dyn-*).
