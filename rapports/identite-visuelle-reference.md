...
- **Cartes de source — « disposition A » full-art** (refonte 2026-07-25, cf. bloc en fin de `site.css`) : ratio **2/3**, radius **18/14** (double liseré violet translucide), fond dégradé thémé + **motif SVG plein-carte** (injecté par `cards.js`, `FULLART_SVG`, remplaçable par catégorie plus tard) + **voile de lisibilité** dégradé. Rangée haute : pastille d'état (verre dépoli) à gauche, contrôles ronds 26px à droite dans l'ordre **♥ · partage · + deck · i** (le « + deck » teinté violet). Contenu ancré en bas : titre Baloo 2 20px + coche, sous-titre violet uppercase 10.5px, description 12px, chips paramètres (pilules verre + « + ajouter » violet plein), interrupteur. Variables `--tc-*` (thème jour = light / nuit = dark) centralisent couleurs/opacités/voile. Scope `.grid` + `.deck-card` (le deck-stack réutilise le même composant). Système de retournement 3D conservé (versos ré-habillés même langage). La carte « Mon compte » (`.acct-*`) et l'aperçu dev (`.preview`) restent hors scope (radius 24px classique).
